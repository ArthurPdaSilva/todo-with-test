...
const renderInput = (p?: Props) => {
  const result = render(makeInput(p));
...
describe("<InputText />", () => {
  describe("comportamento padrão", () => {
    ...
    test("renderiza sem placeholder", () => {
      const el = input({ placeholder: undefined });
      expect(el).not.toHaveAttribute("placeholder");
    });

    test("renderiza sem label", () => {
      input({ labelText: undefined });
      // const label = screen.getByText("novo label"); //Vai falhar pq ele lança erro se não tá na tela
      const label = screen.queryByRole("novo label");
      expect(label).not.toBeInTheDocument();
    });

    test("usa labelText como aria-label quando possível", () => {
      expect(input()).toHaveAttribute("aria-label", "label");
    });

    test("usa placeholder como fallback de aria-label", () => {
    ...
    test("exibe o valor padrão corretamente", () => {
      expect(input({ defaultValue: "qualquer" })).toHaveValue("qualquer");
    });

    test("aceita outras props do JSX (name, maxLength)", () => {
  ...
  describe("acessibilidade", () => {
    test("não exibe mensagem de erro por padrão", () => {
      const el = input();
    ...
    test("não marca o input como inválido por padrão", () => {
      const el = input();
      expect(el).toHaveAttribute("aria-invalid", "false");
    });

    test("renderiza mensagem de erro quando `errorMessage` é passada", () => {
      const el = input({ errorMessage: "Erro aconteceu" });
